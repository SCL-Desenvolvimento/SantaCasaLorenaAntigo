					...
					_thumbCreator: {
						...
						renderFile: function(file, html, opts) {
							...
							if(file.file && opts.type == "image") {
								var g = '<img src="' + file.file + '" class="img-responsive" draggable="false" />',
							...
							if(window.File && window.FileList && window.FileReader && opts.type == "image" && opts.size < 6e+6) {
								...
								y.onload = function(e) {
									var g = '<img src="' + e.target.result + '" class="img-responsive" draggable="false" />',
										m = html.find('.jFiler-item-thumb-image.fi-loading');
									$(g)
										.error(function() {
											g = f._thumbCreator.generateIcon(opts);
						...
						_box: function(params) {
							...
							if(l.length < 1) {
								...
								//alterado - para adicionar thumbs provenientes dos dois inputs 
								//appendTo.find('.jFiler-items')
									//.remove();
								l = $('<div class="jFiler-items jFiler-row col-lg-12 col-md-12 col-sm-12 col-sx-12"></div>');
								s.prop("jFiler").listEl = l;
