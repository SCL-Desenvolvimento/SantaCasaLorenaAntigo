...
- HTTP: `node tests/security-http.cjs CAMINHO_PHP`, 86 verificações; requer PDO SQLite, Fileinfo e GD na pasta `ext` do runtime de teste.
...
- O restante do administrativo ainda usa bibliotecas antigas. Esta etapa protege os fluxos revisados; não equivale a certificação de ausência de vulnerabilidades nem conclui toda a modernização tecnológica do site.
