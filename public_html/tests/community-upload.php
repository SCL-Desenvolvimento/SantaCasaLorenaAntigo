...
$response=array('success'=>$result['success'],'errors'=>$result['errors'],'mime_attachment'=>str_contains(UploadTestMailer::$mime,'curriculo.pdf'),'random_pdf'=>preg_match('~^arquivos/curriculuns/[a-f0-9]{48}\.pdf$~',$stored)===1,'stored'=>($stored && is_file(DIR.$stored)));
if($stored && is_file(DIR.$stored))unlink(DIR.$stored);
