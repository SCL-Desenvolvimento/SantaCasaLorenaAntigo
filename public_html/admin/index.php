    ...
    switch ($_POST['action'] ?? '') {
        case 'login':
            if (scl_authenticate((string) ($_POST['usuario'] ?? ''), (string) ($_POST['senha'] ?? ''))) {
                header('Location: ' . ROOT . 'admin/painel.php', true, 303); exit;
            }
...
<link rel="stylesheet" href="<?= login_escape(ROOT . 'resources/css/admin-login.css') ?>"></head><body><aside class="login-story"><a href="<?= login_escape(ROOT) ?>" aria-label="Santa Casa de Lorena"><img src="<?= login_escape(ROOT . 'resources/img/logo-footer.svg') ?>" alt="Santa Casa de Lorena"></a><span>ADMINISTRAÇÃO DO SITE</span><h2>Cuidar da informação.<br>Conectar pessoas.</h2><p>Um espaço para manter a comunidade informada e os canais de atendimento próximos de quem precisa.</p></aside><main>
...
<form method="post" action="<?= login_escape(ROOT . 'admin/index.php') ?>">
